...
ENV VIRTUAL_ENV /opt/fridge-app-backend-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/fridge-app-backend/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/fridge-app-backend/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/fridge_app_backend/ && touch src/fridge_app_backend/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/fridge-app-backend/
...
# Expose the app.
ENTRYPOINT ["/opt/fridge-app-backend-env/bin/poe"]
CMD ["api"]
